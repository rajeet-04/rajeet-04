...
export function ArchiveSection() {
  ...
  return (
    <section id="archive" className="section archive-section" aria-labelledby="archive-title">
      ...
      {visible.length === 0 ? <p className="empty-state">No repositories match this filter. Browse the <a href="https://github.com/rajeet-04">GitHub profile ↗</a>.</p> : <div className="archive-list">{visible.slice(0, 10).map((project) => <a className="archive-row" key={project.name} href={project.htmlUrl} target="_blank" rel="noreferrer"><strong>{project.name}</strong><span>{project.language ?? 'Unspecified'} · {project.ownership}{project.archived ? ' · archived' : ''}</span></a>)}</div>}
      {visible.length > 10 && <p className="archive-note">Showing 10 of {visible.length}; browse the full archive on <a href="https://github.com/rajeet-04" target="_blank" rel="noreferrer">GitHub ↗</a>.</p>}
    </section>
  );
}
