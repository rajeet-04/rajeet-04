...
export function HeroSection({ ready = true }: { ready?: boolean }) {
  ...
  return <section ref={ref} className="hero section" aria-labelledby="hero-title">
    ...
    <div className="hero__map"><div className="hero__map-content"><PretextStatement /><ul aria-label="Engineering focus">{focus.map((item) => <li key={item}><a href={item === 'Research' ? '#research' : item === 'Robotics exploration' ? '#capabilities' : '#work'}>{item}</a></li>)}</ul></div></div>
  </section>;
}
